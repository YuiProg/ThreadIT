...
class Modal extends React.Component<ModalState, UpdateInput> { 
  ...
  render(): React.ReactNode {
    ...
    return (
      <>
       <div onClick={() => this.props.onClose()} className="inset-0 bg-black/40 z-50 fixed flex items-center justify-center">
          <div onClick={(e) => e.stopPropagation()} className="w-150 h-150 bg-base-200 rounded-xl shadow-2xl flex flex-col items-center p-20">
            {this.user.profilePic || this.state.image
            ? (
              <div className="w-40 h-40 rounded-full border flex items-center justify-center mb-10">
                <img loading="lazy" src={this.state.image ? this.state.image : this.user.profilePic} alt="profilePre"  className="relative w-40 h-40 rounded-full object-cover"/>
              </div>
            ) 
            : (
              <div className="w-40 h-40 rounded-full border flex items-center justify-center mb-10">
            ...
            <form onSubmit={(e) => this.onSubmit(e)} className="w-full">
              <input onChange={(e) => this.handleImage(e)} type="file" accept="image/png, image/jpeg" className="file-input mb-5 w-full" />
              ...
              />
              <button disabled={authenticating} type="submit" className="btn btn-dash w-full">{authenticating ? <span className="loading loading-spinner loading-md"></span> : 'UPDATE' }</button>
            </form>
          </div>
       </div>
      </>
        ...
        {this.state.isOpen && <Modal user={{user: this.props.user}} isOpen={this.state.isOpen} onClose={() => this.setState({isOpen: false})}/>}
          ...
          <div className="fixed w-full z-10">
            <nav className="w-full h-16 bg-gray-800 flex items-center justify-between">
              <div>
                <h1 className="text-white text-2xl font-bold ml-5"><a href="/">THREAD.IT</a></h1>
              </div>
              <div className="w-[50%] flex items-center justify-end">
                ...
                {this.props.user ? (
                  ...
                  <div className="dropdown dropdown-end flex">
                    ...
                    ? (
                      <>
                      <div tabIndex={0} role="button" className="w-10 h-10 border mr-5 text-xl cursor-pointer rounded-full flex items-center justify-center">
                        <div className="w-10 h-full rounded-full absolute hover:bg-black/20 transition-colors z-99"/>
                        <img loading="lazy" src={this.props.user.profilePic} alt="profilePic" className="w-10 h-10 rounded-full object-cover"/>
                      </div>
                      </>
                      
                    ) 
                    : (
                      <div tabIndex={0} role="button" className="w-10 h-10 border mr-2 text-xl cursor-pointer rounded-full flex items-center justify-center">
                        <User/>
                      </div>
                    )}
                    <ul tabIndex={0} className="dropdown-content menu bg-base-200 rounded-box z-1 w-52 p-2 mt-15 mr-2 shadow-sm">
                      <li><a>PROFILE</a></li>
                      <li onClick={() => this.setState({isOpen: true})}><a>EDIT PROFILE</a></li>
                ...
                <ul className="flex gap-5 mr-5">
                  <li><a href="/" className="text-white text-lg hover:text-gray-400">Login</a></li>
                </ul>
                )}
              </div>
            </nav>
